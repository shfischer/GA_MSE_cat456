...
```
This is branch `harvest_rate_R4.2` in which R and R packages have been updated to R 4.2 (4.3 should also work):
```{r, eval = FALSE}
> sessionInfo()
...
The framework is based on the Fisheries Library in R (FLR) framework and uses the [FLR packages](https://flr-project.org/) `FLCore`, `FLash`, `FLBRP`, `ggplotFL`, `mse`. See [renv.lock](https://github.com/shfischer/GA_MSE_HR/blob/harvest_rate_R4.2/renv.lock) for version details and sources.
