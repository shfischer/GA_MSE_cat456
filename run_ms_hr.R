...
if (FALSE) {
  ...
  source("funs_GA.R")
  source("funs_OM.R")
  ### create input for MSE
  input <- input_mp(stocks = "pol",
                    fhist = "one-way",
                    MP = "hr", 
                    idx_sel = "tsb", ### total stock biomass index
                    hr_value = 1, ### harvest rate target relative to tsb
                    multiplier = 0.5 ### adjust target
  )
  ### run MSE
  res <- do.call(mp, input$pol)
  
  ### same example but run with run_ms_hr.R script
  args_local <- c("fhist='one-way'", "stock_id=12",
                  "MP='hr'", "hr=1", "multiplier=0.5")
  ### then run run_ms_hr.R, e.g. with 
  source("run_ms_hr.R")
  ### or line by line
  
  ### find optimum multiplier (restart R after running code above)
  ### this runs all multipliers from 0 to 2 by 0.01
  args_local <- c("fhist='one-way'", "stock_id=12",
                  "MP='hr'", "hr=1", "multiplier=TRUE", 
                  "ga_search=TRUE", "popSize=201", "maxiter=1", 
                  "scenario='multiplier_search'")
  ### then run run_ms_hr.R, e.g. with 
  source("run_ms_hr.R")
  ### or line by line
}
...
hr_params <- c(
  ### OM
  "stock_id", "fhist", "n_iter", "n_yrs", "scenario", "n_blocks",
  ### MP 
  "MP",
  ### uncertainty
  "sigmaL", "sigmaL_rho", "sigmaB", "sigmaB_rho", "sigmaC", 
  "sigmaR", "sigmaR_rho", "steepness", "sigmaIEM",
  ### MP parameters
  "hr", "multiplier", "comp_r", "comp_f", "comp_b", "exp_b", 
  "comp_b_multiplier", "interval", "idxB_lag", "idxB_range_3",
  "upper_constraint", "lower_constraint", "cap_below_b"
)
...
if (isFALSE(ga_search)) {
  ...
  . <- foreach(hr_i = seq(nrow(hr_params))) %do_tmp% {
    ...
    ### generate file name
    pars_str <- par_i[-1:-7]
    file_out <- paste0(pars_str, collapse = "_")
